...
local function CraftItems(item)
    QBCore.Functions.Progressbar('crafting', 'CRAFTING '..Config.Main[item].label, 10000, false, false, {
        ...
        TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items[Config.Main[item].itemName], "add", 1)
        for k, v in pairs(Config.Main[item].items) do
            TriggerServerEvent('QBCore:Server:RemoveItem', v.item, v.amount)
            TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items[v.item], "remove", v.amount)
        end
...
RegisterNetEvent('mt-crafting:client:CraftItems', function(data)
    QBCore.Functions.TriggerCallback("mt-crafting:server:ItemsSuficientes", function(hasItems)
        if (hasItems) then
            ...
            QBCore.Functions.Notify("You need more materials!", "error")
            return
        end
...
RegisterNetEvent('mt-crafting:client:SpawnarMesa', function()
    ...
    local x, y, z   = table.unpack(coords + forward * 1.4)

    local model = `prop_tool_bench02_ld` ----which object models can be used for crafting
    ...
    TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items['mesa_craft'], "remove", 1)
end)
...
RegisterNetEvent('mt-crafting:client:EliminarMesa', function()
    ...
    if DoesEntityExist(obj) then
        ...
        TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items['mesa_craft'], "add", 1)
    end
end)
